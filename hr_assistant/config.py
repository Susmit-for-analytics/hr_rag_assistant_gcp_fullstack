...
LOCATION = os.getenv("LOCATION")  # default GCP region for this project's resources
# The Gemini call does NOT read LOCATION — it has its own knob, LLM_LOCATION
# (under MODELS below), because the model and the location it is served from
...
# A model swap is LLM_MODEL_NAME + LLM_LOCATION together, never the model
# alone: a Gemini model is only reachable from the locations Google serves it
# from, and asking a location that doesn't serve it fails as a 404 "model not
# found" — which reads like a typo'd model id, not a region problem.
#
#   gemini-3.x (incl. gemini-3.1-flash-lite)  -> LLM_LOCATION=global only
#   gemini-2.5-flash                          -> global or a region
#                                                (us-central1, ...), but it
#                                                retires ~2026-10-20
#
# Which locations serve which model id:
# https://docs.cloud.google.com/vertex-ai/generative-ai/docs/learn/model-versions
#
# "global" is not a region — it is Vertex's global endpoint
# (aiplatform.googleapis.com, no region prefix), which is also what gives the
# newer models capacity across regions instead of one region's quota.
# hr_assistant/llm.py prefixes the id with "vertex_ai/" and passes
# LLM_LOCATION straight through as vertex_location.
LLM_MODEL_NAME = os.getenv("LLM_MODEL_NAME", "gemini-3.1-flash-lite")
LLM_LOCATION = os.getenv("LLM_LOCATION", "global").strip() or "global"
